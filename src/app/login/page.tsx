...
function LoginForm() {
  ...
  if (signupEmail) {
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-coral-100 p-8 text-center">
        <p className="text-3xl mb-3">📬</p>
        <p className="text-lg font-semibold text-[#1A0A00] mb-2">check your email</p>
        <p className="text-sm text-[#1A0A00]/60 leading-relaxed">
          we sent a confirmation link to{' '}
          <span className="font-medium text-[#1A0A00]">{signupEmail}</span>.
          <br />
          click it and you'll be brought back to sign in.
        </p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-coral-100 p-6">
      {confirmed && (
        <div className="mb-5 px-4 py-3 bg-green-50 border border-green-200 rounded-xl text-sm text-green-700 text-center font-medium">
      ...
      <div className="flex rounded-xl bg-coral-100 p-1 mb-6">
        <button
          ...
          className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
            mode === 'login' ? 'bg-white text-coral-400 shadow-sm' : 'text-[#1A0A00]/60'
          }`}
        >
          sign in
        </button>
        <button
          ...
          className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
            mode === 'signup' ? 'bg-white text-coral-400 shadow-sm' : 'text-[#1A0A00]/60'
          }`}
        >
          create account
        </button>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label className="block text-xs font-medium text-[#1A0A00]/60 mb-1.5">password</label>
          <input
            type="password"
            value={password}
